...
int main() {
    ...
    for (int i = 0; i < numVertices; ++i) {
        ...
        for (int j = 0; j < edgesToAdd; ++j) {
            ...
            if (target != i) {  //Impede arestas de laço (vértice se conectando a ele mesmo)
                //Verifica se já existe uma aresta entre os dois vértices
                bool edgeExists = false;
                for (auto& e : g.edges) {  //Usando 'edges' diretamente
                ...
                if (!edgeExists) {  //Se não existe, adicione a aresta
                    ...
                    g.addEdge(i, target, distance, trafficRate);
                }
            }
        }
    }
